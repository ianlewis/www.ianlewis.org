...
![A screenshot of the Jaiku top page](/assets/images/516/jaiku.png){: .align-center}

Yesterday Google's Twitter-like service, Jaiku was
[released](http://code.google.com/p/jaikuengine/) as open source running on
[Google App Engine](https://cloud.google.com/appengine) so I decided to take it
...
At first I tried just zipping the files up using the `zip_all` recipe the
`Makefile` (`make zip_all`) but I still got the same error so I just commented
out the relevant parts in `app.yaml`.
