...
[`Backbone.js`](http://backbonejs.org/) is a neat little JavaScript model
...
[jQuery deferred](http://api.jquery.com/category/deferred-object/). This allows
you to do cool things like doing work in parallel and then running a callback
when all the work is done.

Let's look at an example. Say we have two models we want to save to the server
simultaneously but we only want to update the UI when both are finished. We can
use jQuery deferred objects to set up the pipeline.
...
Of course, you can do lots of more powerful things with jQuery deferred objects
but the fact that `Backbone.js` allows you to do use them may not be immediately
obvious.
