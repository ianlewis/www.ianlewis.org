...
There is a lot of activity around AI IDEs right now.

Many developers seem to be getting a lot of value out of these editors, but
there also seems to be a fair amount of hype. All this activity has me thinking:
What about AI necessitates having a new IDE? Why are these IDEs a fork of VSCode
and not an extension? Why are there so many? What do developers actually get out
of it? Do we even need these new IDEs?
...
editor. This by itself might be good, but depending on the features you use, you
might be able to get that experience using plugins or extensions.
...
While this is very nice to have when working with AI, almost all the VSCode
...
Almost all code editors have some kind of extension that allows you to do this
even before AI IDEs started being released. We’ve had auto-complete snippets
plugins and LSP-like introspection in IDEs for a long time, so it’s obviously
not necessary to use a whole new editor to make use of AI-generated
autocomplete.
...
This is convenient, but it’s like candy bars at the register in the supermarket.
It doesn’t have anything to do with the editor itself. This has also existed as
a convenience within IDEs for
...
for that experience if they know that is what they are paying for, and that
AI model features are available outside the editor.
...
may not, by themselves, be the thing to lock-in developers, but I think that
they could set the stage for a new round of software vendor lock-in.
...
need so many AI IDEs all of a sudden. Why they are getting multi-billion
...
I wonder why companies think we even need editors or IDEs. Shouldn't a chat box
be all we need?
...
not. AI IDEs may be good for new developers getting into the field to make
use of AI. Those developers may not care about customizing their IDE as much as
existing developers.

For existing developers, I’m skeptical that using a forked IDE is the way
